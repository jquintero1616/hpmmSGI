...
const Kardex: React.FC<{ status: string }> = ({ status = "Todo" }) => {
  ...
  const kardexColumns: Column<KardexDetail>[] = [
    {
      header: "Factura",
      accessor: (row) => row.shopping_order_id || "N/A",
    },
    {
      header: "Producto",
    ...
    {
      header: "Unidad",
      accessor: (row) => row.nombre_unidad || "N/A",
    },
    {
    ...
    },
    {
      ...
      accessor: (row) => (
        <span className="font-semibold text-blue-600">
          L.{Number(row.total).toFixed(2)}
        </span>
      ),
    },
    {
      header: "Vendedor",
      accessor: (row) => row.vendedor_nombre || "N/A",
    },
    {
      header: "Fecha de Compra",
      accessor: (row) =>
        row.fecha_compra
          ? new Date(row.fecha_compra).toLocaleDateString()
          : "N/A",
    },
  ];
  ...
  const kardexListColumns: Column<KardexRow>[] = [
    ...
    },
    {
      ...
      accessor: (row) => (
        <span className="font-medium">
          {Number(row.cantidad_recepcionada).toFixed(2)}
        </span>
      ),
      editable: true,
    },
    {
      header: "Observación",
      accessor: (row) => row.observacion || "N/A",
      editable: true,
    },
    {
      header: "Descripción",
      accessor: (row) => row.descripcion || "N/A",
      editable: true,
    },
    {
    ...
    },
    {
      ...
      accessor: (row) => row.numero_lote || "N/A",
      editable: true,
    },
    {
      header: "Unidad",
    ...
    },
    {
      ...
      accessor: (row) => row.tipo_movimiento || "N/A",
    },
    {
      header: "RFID",
      accessor: (row) => row.RFID || "",
      editable: true,
    },
  ];
  ...
  const cargarProductosDeCompra = async (id_scompra: string) => {
    try {
      ...
      const productosFormateados = compra.map((producto: any) => ({
        ...
        shopping_order_id: producto.shopping_order_id,
    ...
    try {
      ...
      for (const item of dataListForm) {
        if (
          ...
          await PutUpdateKardexContext(item.id_shopping, item);
        } else {
